...
/**
 * Creates a filter that only keeps events related to a specific object.
 * @param objectId Object ID to keep.
 * @returns A filter which keeps events related to the given object.
 */
export function filterByObject(ctx: ExtContextData, objectId: number): Filter<{}> {
    return initFilter(ctx, {
        title: `Show only events with object ${objectId}`,
        enabled: true,
        builtin: false,
        data: {},
        check: (_self, {category, data}) => category === "semaphore" && data.objectId === objectId,
    });
}
